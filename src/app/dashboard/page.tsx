...
export default function DashboardPage() {
  ...
  const updateRequest = async (id: string, status: "accepted" | "rejected") => {
    const response = await axiosPrivate.patch(`/api/swapRequest/${id}`, {
      status,
    });
  ...
  const tabs = [
    { id: "overview" as DashboardTab, label: "Overview", icon: Sparkles },
    {
      id: "received" as DashboardTab,
      label: "Requests",
      icon: Inbox,
      count: pendingCount,
    },
    ...
    <div className="min-h-screen bg-slate-50">
      <main className="mx-auto max-w-6xl px-5 py-9 sm:px-6 lg:px-8">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-black tracking-[-0.045em] text-slate-950 sm:text-3xl">
            Swap center
          </h1>
          <p className="mt-2 text-sm font-medium text-slate-500">
            Manage requests, conversations, and upcoming learning sessions.
          </p>
              ...
              {!hasOverviewContent ? (
                <section className="rounded-[26px] border border-dashed border-slate-300 bg-white px-6 py-16 text-center">
                  ...
                  <h2 className="mt-5 text-xl font-black text-slate-950">
                    Your first swap starts in Discover
                  </h2>
                  <p className="mx-auto mt-2 max-w-md text-sm font-medium leading-6 text-slate-500">
                    Find someone whose skills complement yours, then send a
                    thoughtful request.
                  </p>
                  <Link
                    href="/discover"
                    className="mt-6 inline-flex h-11 items-center gap-2 rounded-xl bg-blue-600 px-5 text-sm font-extrabold text-white"
                  >
                    Explore members <ChevronRight className="h-4 w-4" />
                  </Link>
                </section>
              ) : (
          ...
          {activeTab === "chats" &&
            (chatRooms.length === 0 ? (
              <EmptyState
                ...
                description="A private conversation is created after a request is accepted."
                icon={MessageCircleMore}
              />
            ) : (
              <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                {chatRooms.map((room, index) => {
                  const otherUser =
                    dashboard.user.id === room.swapRequest.sender.id
                  ...
                  return (
                    <Link
                      key={room.id}
                      href={`/chat/${room.id}`}
                      className={`flex items-center gap-3 px-4 py-4 transition hover:bg-slate-50 sm:gap-4 sm:px-5 sm:py-5 ${index > 0 ? "border-t border-slate-100" : ""}`}
                    >
                      <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-blue-100 text-sm font-black text-blue-700 sm:h-12 sm:w-12">
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
      {requests.map((request, index) => {
        ...
        return (
          <article
            key={request.id}
            className={`p-5 sm:p-6 ${index > 0 ? "border-t border-slate-100" : ""}`}
          >
            <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
              <div className="flex min-w-0 flex-1 items-center gap-4">
                <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-blue-100 text-sm font-black text-blue-700">
                  {initials(otherUser.name)}
                </span>
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span
                      className={`rounded-full px-2.5 py-1 text-[10px] font-black capitalize ${statusStyles[request.status] || "bg-slate-100 text-slate-600"}`}
                    >
                      {request.status}
                    </span>
                  </div>
              ...
              <div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:shrink-0">
                {request.status === "pending" && onAccept && onReject && (
                  <>
                    <button
                      ...
                      className="inline-flex h-10 items-center gap-2 rounded-xl border border-slate-200 px-4 text-sm font-extrabold text-slate-600"
                    >
                ...
                {request.status === "accepted" && (
                  <span className="col-span-2 inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-emerald-50 px-4 text-sm font-extrabold text-emerald-700">
                    <Clock3 className="h-4 w-4" /> Active swap
                  </span>
                )}
              </div>
            </div>
